...
BSM_Analysis::BSM_Analysis(TFile* theFile, TDirectory *cdDir[], int nDir, char* fname)
{
	...
	for (int i = 0; i < nentries; ++i)
	{
		...
		hzg_first_lep_vec.erase (hzg_first_lep_vec.begin(),hzg_first_lep_vec.end());
		vector<TLorentzVector> hzg_Subfirst_lep_vec;
		hzg_Subfirst_lep_vec.erase (hzg_Subfirst_lep_vec.begin(),hzg_Subfirst_lep_vec.end());
		...
		for (int m1 = 0; m1 < Muon_pt->size(); m1++)
		{
			...
			if ((Muon_pt->size() > 1) && (pass_trigger) && (Muon_pt->at(m1) > 20.0))
			{				
				if((MuonIso1>0.25)&& (Muon_tight->at(m1)==1))
				{
					lepton1_evt++;
					...
					for (int m2 = 0; m2 < Muon_pt->size(); m2++)
					{
						if ((Muon_pt->size() > 1) && (Muon_pt->at(m2) >15.0))
						{
							...
							if((MuonIso2>0.25)&& (Muon_tight->at(m2)==1))
							{
								if (Muon_charge->at(m1)*Muon_charge->at(m2) < 0){
									...
									if (dimuon_mass < dimuon_mass_int ){
										...
										if (dimuon_mass_int >50)
										{
											hzg_first_muon_vec.SetPtEtaPhiE(Muon_pt->at(m1), Muon_eta->at(m1), Muon_phi->at(m1), Muon_energy->at(m1));
											hzg_first_lep_vec.push_back(hzg_first_muon_vec);
											hzg_Subfirst_muon_vec.SetPtEtaPhiE(Muon_pt->at(m2), Muon_eta->at(m2), Muon_phi->at(m2), Muon_energy->at(m2));
											hzg_Subfirst_lep_vec.push_back(hzg_Subfirst_muon_vec);
											dimuon_evt++;
										}
									}
									lepton2_evt++;	
		...
		Dilep_vec = hzg_first_muon_vec + hzg_Subfirst_muon_vec;
		
		for(int tb = 0; tb < Photon_vec.size(); tb++)
		{
			if((Photon_vec.size() > 0) &&( hzg_first_lep_vec.size() > 0) ) 
			{
				Photon_sel_vec = Photon_vec.at(tb);
				float delta = Dilep_vec.DeltaR(Photon_sel_vec);
				float Mllg = (hzg_first_muon_vec + hzg_Subfirst_muon_vec + Photon_sel_vec).M();
				float Mll = (hzg_first_muon_vec + hzg_Subfirst_muon_vec).M();
				
				if ( (delta >0.4) && ( (Photon_vec.at(tb).Pt())/Mllg>0.1363) && Mll + Mllg >185)
				{
					photon_evt ++;
				}
			}
	...
	cout<<"Events that pass lepton 1 cuts:    "<<lepton1_evt<<endl;
	cout<<"Events that pass lepton 2 cuts:    "<<lepton2_evt<<endl;
	cout<<"Events that pass the dimuon selection:   "<<dimuon_evt<<endl;
	cout<<"Events that pass photon selection:    "<<photon_evt<<endl;
